...
def regression():
    lesson('R01','Read a fitted line','Interpret slope and intercept in context.',
        ...
        py('Read the fitted distance coefficient and intercept.',"answer={'slope':float(model.coef_[0]),'intercept':float(model.intercept_)}","np.isclose(answer['slope'],model.coef_[0]) and np.isclose(answer['intercept'],model.intercept_)",setup=FIT),
    ...
    lesson('R02','RMSE and R² answer different questions','Distinguish target-unit errors from relative fit.',
        ...
        py('Compute RMSE and R² for the supplied predictions.',"from sklearn.metrics import root_mean_squared_error,r2_score\nanswer={'RMSE':root_mean_squared_error(y_test,predictions),'R2':r2_score(y_test,predictions)}","np.isclose(answer['RMSE'],np.sqrt(np.mean((y_test-predictions)**2))) and np.isclose(answer['R2'],1-np.sum((y_test-predictions)**2)/np.sum((y_test-y_test.mean())**2))",setup=FIT),
        ...
        py('Compare the evaluation mean and fitted training mean.',"answer={'evaluation_mean':float(y_test.mean()),'training_mean':float(y_train.mean())}","np.isclose(answer['evaluation_mean'],y_test.mean()) and np.isclose(answer['training_mean'],y_train.mean())",setup=SPLIT)])
...
"""
    ...
    lesson('R06','Correlated predictors and unstable coefficients','Separate coefficient stability from predictive stability.',
        ...
        py('Fit near-duplicate distance predictors and inspect their coefficients.',"from sklearn.linear_model import LinearRegression\nrng=np.random.default_rng(42)\nX=pd.DataFrame({'distance':df.distance,'near_copy':df.distance+rng.normal(0,.001,len(df))})\nmodel=LinearRegression().fit(X,df.duration)\nanswer=pd.Series(model.coef_,index=X.columns)","len(answer)==2 and np.isfinite(answer).all()",dataset='MIX60'),
...
"""
    lesson('R08','Validate polynomial flexibility','Choose degree inside the complete pipeline.',
        ...
        py('Create the polynomial pipeline and inspect the expanded training dimensions.',poly+"answer=model.named_steps['polynomial'].fit_transform(X_train)","answer.shape==(len(X_train),2)",dataset='CURVE48',setup=CURVE),
    ...
    lesson('R09','Trees predict with leaf averages','Learn recursive splits and regression leaves within this branch.',
        ...
        py('Find the leaf for x=5 and its prediction.',"row=pd.DataFrame({'x':[5.]})\nanswer={'leaf':int(model.apply(row)[0]),'prediction':float(model.predict(row)[0])}","np.isclose(answer['prediction'],model.tree_.value[answer['leaf']].ravel()[0])",dataset='STEP60',setup=CURVE+"\nfrom sklearn.tree import DecisionTreeRegressor\nmodel=DecisionTreeRegressor(max_depth=3,random_state=42).fit(X_train,y_train)"),
...
def classification():
    ...
    lesson('C02','Precision and recall','Connect different errors to different questions.',
        ...
        py('Calculate precision and recall for B.',"from sklearn.metrics import precision_score,recall_score\nanswer=[precision_score(df.actual,df.predicted,pos_label='B'),recall_score(df.actual,df.predicted,pos_label='B')]","np.allclose(answer,[.5,.25])",dataset='ERROR12'),
        ...
        py('Return precision and recall for both A and B in explicit order.',"from sklearn.metrics import precision_score,recall_score\nanswer=pd.DataFrame({'precision':precision_score(df.actual,df.predicted,labels=['A','B'],average=None),'recall':recall_score(df.actual,df.predicted,labels=['A','B'],average=None)},index=['A','B'])","np.allclose(answer.precision,[.7,.5]) and np.allclose(answer.recall,[.875,.25])",dataset='ERROR12')])
    lesson('C03','Macro F1 and imbalance','Evaluate every class explicitly.',
        ...
        py('Compare macro F1 with accuracy.',"from sklearn.metrics import f1_score,accuracy_score\nanswer={'macro_f1':f1_score(df.actual,df.predicted,average='macro'),'accuracy':accuracy_score(df.actual,df.predicted)}","np.isclose(answer['macro_f1'],5/9) and np.isclose(answer['accuracy'],2/3)",dataset='ERROR12'),
        py('Evaluate a most-frequent classifier on Car’s stratified test split.',"from sklearn.dummy import DummyClassifier\nfrom sklearn.metrics import f1_score,accuracy_score\nmodel=DummyClassifier(strategy='most_frequent').fit(train[['buying']],train.acceptability)\npred=model.predict(test[['buying']])\nanswer={'macro_f1':f1_score(test.acceptability,pred,average='macro'),'accuracy':accuracy_score(test.acceptability,pred)}","0<=answer['macro_f1']<=1 and answer['accuracy']>answer['macro_f1']",dataset='car',setup="from sklearn.model_selection import train_test_split\ntrain,test=train_test_split(df,test_size=.2,random_state=42,stratify=df.acceptability)")])
    ...
    lesson('C05','Logistic regression','Understand a regularised linear class boundary.',
        ...
        py('Inspect multiclass coefficient dimensions and class names.',"answer=pd.DataFrame(model.named_steps['model'].coef_,index=model.classes_,columns=X_train.columns)","answer.shape==(3,2) and list(answer.index)==list(model.classes_)",dataset='CLASS180',setup=CLASS_FIT)],chapter=1,models=['logistic'])
...
"""
    ...
    lesson('C08','Neighbour voting','Explain a classifier’s local distance-based decision.',
        ...
        py('Fit three-neighbour KNN and inspect the neighbours of the first test row.',"from sklearn.neighbors import KNeighborsClassifier\nfrom sklearn.preprocessing import StandardScaler\nscaler=StandardScaler().fit(X_train)\nmodel=KNeighborsClassifier(n_neighbors=3).fit(scaler.transform(X_train),y_train)\nanswer=model.kneighbors(scaler.transform(X_test.iloc[:1]),return_distance=False)","answer.shape==(1,3) and np.all(answer<len(X_train))",dataset='CLASS18',setup=CLASS),
    ...
    lesson('C10','Margins and support vectors','Interpret the separating boundary and supporting examples.',
        ...
        py('Inspect the fitted support-vector count for each class.',"answer=model.named_steps['model'].n_support_","len(answer)==3 and np.all(answer>0)",dataset='CLASS180',setup=svc),
...
"""
    lesson('C12','One feature, one rule','Understand the production One-R rule and fallback.',
        ...
        py('Fit a discrete One-R rule and inspect its selected feature and rules.',"answer=model.named_steps['model'].rules_","isinstance(answer,dict) and len(answer)>0",dataset='RULE24',setup=rules),
...
"""
    ...
    lesson('C14','Classification trees','Learn recursive splits, impurity, class leaves and validation within Classification.',
        ...
        py('Fit a depth-three tree, trace the first test row to a leaf and predict it.',"from sklearn.tree import DecisionTreeClassifier\nmodel=DecisionTreeClassifier(max_depth=3,random_state=42).fit(X_train,y_train)\nanswer={'leaf':int(model.apply(X_test.iloc[:1])[0]),'label':model.predict(X_test.iloc[:1])[0]}","answer['label'] in set(y_train) and answer['leaf']>=0 and model.get_depth()<=3",dataset='CLASS180',setup=CLASS),
    ...
    lesson('C17','LDA shares a covariance shape','Understand shared class geometry and linear boundaries.',
        ...
        py('Fit LDA and inspect class means.',"answer=pd.DataFrame(model.means_,index=model.classes_,columns=X_train.columns)","answer.shape==(2,2) and np.allclose(answer.loc['A'],X_train.loc[y_train.eq('A')].mean())",dataset='COV90_SHARED',setup=lda),
    ...
    lesson('C18','QDA allows class-specific shapes','Balance covariance flexibility against data support and stability.',
        ...
        py('Compare production regularisation values and inspect any warnings.',"from sklearn.model_selection import GridSearchCV,StratifiedKFold\nsearch=GridSearchCV(QuadraticDiscriminantAnalysis(),{'reg_param':[.1,.2,.5,.9]},cv=StratifiedKFold(5,shuffle=True,random_state=42),scoring='f1_macro').fit(X_train,y_train)\nanswer=search.cv_results_['mean_test_score']","len(answer)==4 and np.isfinite(answer).all()",dataset='COV90',setup=qda),
