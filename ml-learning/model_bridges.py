...
def _hierarchy(card):
    ...
    e=py('Build a complete sampled Ward workflow: scale the declared measurements, compare two-to-four-group cuts, choose one using the evidence and describe its groups in original units.',HIERARCHY_SOLUTION,checks,dataset='CLUSTER36',outputs=['cut_scores','chosen_k','profiles'])
...
def _pca(card):
    ...
    e=py('Build a complete PCA representation: scale measurements, fit PCA, retain the smallest prefix reaching 90% variance and label the first two component weights.',PCA_SOLUTION,checks,dataset='PCA48',outputs=['ratios','retained','reduced','weights'])
