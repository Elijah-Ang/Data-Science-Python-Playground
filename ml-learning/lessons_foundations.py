...
def author():
    ...
    lesson('F03','What fitting does','Recognise learned state from examples.',
        ...
        py('Fit a new line on this 24-row batch using distance to predict duration. Store its distance coefficient in answer.',"from sklearn.linear_model import LinearRegression\nmodel=LinearRegression().fit(df[['distance']],df.duration)\nanswer=float(model.coef_[0])","np.isclose(answer,np.cov(df.distance,df.duration,ddof=0)[0,1]/np.var(df.distance))",dataset='LINE24B')],chapter=1)
    ...
    lesson('F05','Measuring prediction error','Connect residuals and RMSE to target units.',
        ...
        py('Store RMSE for the supplied predictions in original_rmse and RMSE after adding 10 to every prediction in shifted_rmse.',"from sklearn.metrics import root_mean_squared_error\noriginal_rmse=root_mean_squared_error(y_test,predictions)\nshifted_rmse=root_mean_squared_error(y_test,predictions+10)","np.isclose(original_rmse,np.sqrt(np.mean((y_test-predictions)**2))) and np.isclose(shifted_rmse,np.sqrt(np.mean((y_test-predictions-10)**2)))",setup=FIT,outputs=['original_rmse','shifted_rmse']),
...
def workflow():
    lesson('W01','Explore training data','Keep input exploration inside the training population.',
        ...
        py('Count each species in the supplied Penguin training labels y_train and store the counts in answer.',"answer=y_train.value_counts()","answer.equals(y_train.value_counts())",dataset='penguins',setup="from sklearn.model_selection import train_test_split\ntrain,test=train_test_split(df,test_size=.2,random_state=42,stratify=df.species)\ny_train=train.species")])
    lesson('W02','A useful reference','Compare with a simple predictor that ignores X.',
        ...
        py('Store RMSE for the supplied model predictions in model_rmse. Store RMSE for a constant prediction equal to the training-target mean in reference_rmse. Use the same evaluation rows for both.',"from sklearn.metrics import root_mean_squared_error\nmodel_rmse=root_mean_squared_error(y_test,predictions)\nreference_rmse=root_mean_squared_error(y_test,np.repeat(y_train.mean(),len(y_test)))","np.isclose(model_rmse,np.sqrt(np.mean((y_test-predictions)**2))) and np.isclose(reference_rmse,np.sqrt(np.mean((y_test-y_train.mean())**2)))",setup=FIT,outputs=['model_rmse','reference_rmse']),
    ...
    lesson('W04','Encode categories without inventing order','Preserve a reusable categorical schema.',
        ...
        py('Fit a one-hot encoder on training island names. Store the encoded test rows in answer and their feature names in names.',"from sklearn.preprocessing import OneHotEncoder\nencoder=OneHotEncoder(handle_unknown='ignore',sparse_output=False).fit(train[['island']])\nanswer=encoder.transform(test[['island']])\nnames=encoder.get_feature_names_out()","answer.shape[0]==len(test) and np.array_equal(answer,encoder.transform(test[['island']])) and list(names)==list(encoder.get_feature_names_out())",dataset='penguins',setup="from sklearn.model_selection import train_test_split\ntrain,test=train_test_split(df,test_size=.2,random_state=42,stratify=df.species)",outputs=['answer','names'])])
...
"""
    ...
    pipeline=prepare.replace("StandardScaler()", "'passthrough'").replace("sparse_output=False)", "sparse_output=False,drop='first')")+"from sklearn.pipeline import Pipeline\nfrom sklearn.linear_model import LinearRegression\nmodel=Pipeline([('prepare',prepare),('model',LinearRegression())])\n"
...
def validate():
    ...
    lesson('W14','Respect time','Validate in the direction the model will be used.',
        ...
        py('Store the first 80% of TIME240 in train and the final 20% in test, keeping time order.',timeset,"len(train)==192 and len(test)==48 and train.time.max()<test.time.min()",dataset='TIME240',outputs=['train','test']),
        py('Use TimeSeriesSplit to make five forward folds from train. Store the (earlier training positions, later validation positions) pairs in answer.',"from sklearn.model_selection import TimeSeriesSplit\nanswer=list(TimeSeriesSplit(5).split(train))","len(answer)==5 and all(max(a)<min(b) for a,b in answer)",dataset='TIME240',setup=timeset),
