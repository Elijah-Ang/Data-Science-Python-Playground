...
def author():
    lesson('U01','Discovery has no prediction target','State the exploratory population and question.',
        ...
        py('Select Penguin measurements only, excluding species and contextual labels.',"answer=df[['bill_length_mm','bill_depth_mm','flipper_length_mm','body_mass_g']]","list(answer.columns)==['bill_length_mm','bill_depth_mm','flipper_length_mm','body_mass_g'] and len(answer)==len(df)",dataset='penguins'),
    ...
    lesson('U02','Distance depends on scale and context','Fit a meaningful distance representation for the exploratory population.',
        ...
        py('Compare the first pair’s raw and standardised Euclidean distances.',"from sklearn.preprocessing import StandardScaler\nscaled=StandardScaler().fit_transform(df)\nanswer={'raw':float(np.linalg.norm(df.iloc[0]-df.iloc[1])),'scaled':float(np.linalg.norm(scaled[0]-scaled[1]))}","np.isclose(answer['raw'],np.linalg.norm(df.iloc[0]-df.iloc[1])) and np.isclose(answer['scaled'],np.linalg.norm(scaled[0]-scaled[1]))",dataset='CLUSTER36'),
    ...
    lesson('U03','K-Means assigns points to centroids','Understand centroids, distances and assignments.',
        ...
        py('Fit three centroids to scaled CLUSTER36.',"model=KMeans(n_clusters=3,n_init=20,random_state=42).fit(scaled)\nanswer=model.cluster_centers_","answer.shape==(3,2) and np.isfinite(answer).all()",dataset='CLUSTER36',setup=CLUSTER),
        py('Inspect the first row’s assigned centroid and distance to it.',"model=KMeans(n_clusters=3,n_init=20,random_state=42).fit(scaled)\nlabel=int(model.labels_[0])\nanswer={'cluster':label,'distance':float(np.linalg.norm(scaled[0]-model.cluster_centers_[label]))}","np.isclose(answer['distance'],np.linalg.norm(scaled[0]-model.cluster_centers_[answer['cluster']]))",dataset='CLUSTER36',setup=CLUSTER),
...
"""
    lesson('U05','Describe clusters in meaningful units','Use sizes and original-unit profiles with named features.',
        ...
        py('Draw bill length against bill depth with clusters indicated.',"import matplotlib.pyplot as plt\nfig,ax=plt.subplots()\nfor label in np.unique(labels):\n    selected=X.loc[labels==label]\n    ax.scatter(selected.bill_length_mm,selected.bill_depth_mm,label='Group '+str(label),marker=['o','s','^'][int(label)%3])\nax.set(xlabel='Bill length (mm)',ylabel='Bill depth (mm)',title='Groups in two measured features')\nax.legend()\nanswer=X.groupby(labels)[['bill_length_mm','bill_depth_mm']].mean()","answer.shape==(3,2) and len(fig.axes[0].collections)==3",dataset='penguins',setup=penguin),
    ...
    lesson('U06','When K-Means geometry misleads','Recognise initialisation, scale and shape limitations.',
        ...
        py('Compare inertia from three single-start initialisations.',"answer=[]\nfor seed in [1,2,3]:\n    model=KMeans(n_clusters=4,n_init=1,random_state=seed).fit(scaled)\n    answer.append(model.inertia_)","len(answer)==3 and np.isfinite(answer).all()",dataset='CLUSTER36',setup=CLUSTER),
    ...
    lesson('U07','Hierarchical merging','Interpret Ward linkage and merge height.',
        ...
        py('Construct Ward linkage for scaled CLUSTER36.',"answer=linkage_matrix","answer.shape==(len(df)-1,4) and np.all(answer[:,3]>=2)",dataset='CLUSTER36',setup=ward),
    ...
    lesson('U08','Turn a hierarchy into groups','Choose a cut using evidence and purpose.',
        ...
        py('Cut the fitted hierarchy into three groups.',"answer=cut_tree(linkage_matrix,n_clusters=3).ravel()","len(answer)==len(df) and len(np.unique(answer))==3",dataset='CLUSTER36',setup=ward),
        py('Compare cuts yielding two and four groups.',"answer={k:pd.Series(cut_tree(linkage_matrix,n_clusters=k).ravel()).value_counts().sort_index().tolist() for k in [2,4]}","len(answer[2])==2 and len(answer[4])==4 and sum(answer[2])==sum(answer[4])==len(df)",dataset='CLUSTER36',setup=ward),
...
def pca():
    ...
    lesson('P02','Fit a reusable PCA representation','Transform rows with the fitted axes and scale.',
        ...
        py('Fit PCA to scaled PCA48 and return component scores.',"answer=scores","answer.shape==df.shape and np.isfinite(answer).all()",dataset='PCA48',setup=PCA_SETUP),
        ...
        py('Fit a Penguin measurement representation and return scores.',"from sklearn.preprocessing import StandardScaler\nfrom sklearn.decomposition import PCA\nX=df[['bill_length_mm','bill_depth_mm','flipper_length_mm','body_mass_g']]\nscaled=StandardScaler().fit_transform(X)\npca=PCA().fit(scaled)\nanswer=pca.transform(scaled)","answer.shape==(len(df),4) and np.isfinite(answer).all()",dataset='penguins')],models=['pca'])
    ...
    lesson('P04','Select a retained representation','Choose the smallest component prefix meeting a variance criterion.',
        ...
        py('Select the smallest prefix reaching 90% and return the reduced scores.',"cumulative=np.cumsum(pca.explained_variance_ratio_)\nretained=int(np.searchsorted(cumulative,.9)+1)\nanswer=scores[:,:retained]","retained==int(np.searchsorted(np.cumsum(pca.explained_variance_ratio_),.9)+1) and answer.shape==(len(df),retained)",dataset='PCA48',setup=PCA_SETUP),
        py('Compare retained dimensions at 80% and 95%.',"cumulative=np.cumsum(pca.explained_variance_ratio_)\nanswer={threshold:int(np.searchsorted(cumulative,threshold)+1) for threshold in [.8,.95]}","answer[.8]<=answer[.95] and all(answer[t]==int(np.searchsorted(cumulative,t)+1) for t in [.8,.95])",dataset='PCA48',setup=PCA_SETUP),
    ...
    lesson('P06','Two dimensions are a view','Separate a display from the retained representation.',
        ...
        py('Return both a 90%-retained representation and its separate 2D view.',"retained=int(np.searchsorted(np.cumsum(pca.explained_variance_ratio_),.9)+1)\nanswer={'retained':scores[:,:retained],'view':scores[:,:2]}","answer['retained'].shape==(len(df),retained) and answer['view'].shape==(len(df),2)",dataset='PCA48',setup=PCA_SETUP),
...
def comparison():
    ...
    lesson('M03','Explain a result responsibly','Connect claims with evidence and limitations.',
        ...
        py('Build a concise evidence table from the supplied model results.',"answer=pd.DataFrame({'model':['linear','tree'],'cv_rmse':[4.2,4.3],'fold_sd':[.6,.8],'role':['candidate','candidate']})","list(answer.model)==['linear','tree'] and np.allclose(answer.cv_rmse,[4.2,4.3])"),
