...
with sync_playwright() as p:
 ...
 page.evaluate('''() => {
   ...
   for(const selector of ['.notebook-actions','.data-card','.route-strip','article.cell','.output-item']) {
 ...
 for theme in ['light','dark']:
  ...
  for width in [390,834,1440]:
   ...
   assert page.locator('.privacy-note').count()==0
   results.append({'theme':theme,'width':width,'privacy_note':'removed'})
