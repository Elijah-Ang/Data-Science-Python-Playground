...
function run(search = '', saved, blocked = false, protocol = 'https:') {
  ...
  const context = {
    ...
    location:{search, protocol, hostname:'dataplayground.science', origin:'https://dataplayground.science', href:'https://dataplayground.science/'+search},
    ...
    document:{readyState:'complete',documentElement:{dataset:{}},body:{},querySelectorAll:()=>elements},
    MutationObserver:class{observe(){}},
  };
  vm.runInNewContext(source,context);
  return {policy:context.window.DataPlaygroundAds,links:elements.map(item=>item.href),values};
}
for (const blocked of [false,true]) {
  const result=run('?ads=off',undefined,blocked);
  assert.equal(result.policy.adFree,true);
  assert.equal(result.policy.canLoadAdvertising(),false);
  assert.equal(result.links[0],'/playground.html?ads=off');
  assert.equal(result.links[1],'/ml.html?ads=off#workflow');
...
assert.equal(run('', '1').policy.adFree,true);
...
console.log('Ad mode: off/on, tab persistence, blocked storage, native exclusion, links, and all pages passed.');
