...
with sync_playwright() as p:
    for width, height in [(1440, 900), (390, 844)]:
        ...
        # Repeated layout changes replace the prepared texture in one context.
        for next_width,next_height,layout in [(1440,900,'landscape'),(390,844,'portrait')]:
            page.set_viewport_size({'width':next_width,'height':next_height})
            page.wait_for_function("(layout)=>{const scene=document.querySelector('[data-scene]');return scene.dataset.layout===layout&&scene.dataset.motion==='ready';}",arg=layout)
            assert page.locator('.scene-motion').is_visible()
