...
function progress(c){
 ...
 I18S:[null,null],
 I19:[
  d=>[`Display total ${d.a} for each ${d.c} category in df as a Series.`,`df.groupby(${q(d.c)})[${q(d.a)}].sum()`,'An average and a total answer different questions.'],
 ...
 W19:[
  d=>[`Summarise df by ${d.c} with named columns total (${d.a} sum) and n (${d.a} count). Keep ${d.c} as a regular column. Display the summary.`,`df.groupby(${q(d.c)}, as_index=False).agg(total=(${q(d.a)}, "sum"), n=(${q(d.a)}, "count"))`,'Each named aggregation pairs an input field with a summary operation.'],
  d=>[`For df records with ${d.a} at least 2, compare average ${d.b} and group size by ${d.c}. Display columns ${d.c}, average, n.`,`df[df[${q(d.a)}] >= 2].groupby(${q(d.c)}, as_index=False).agg(average=(${q(d.b)}, "mean"), n=(${q(d.b)}, "size"))`,'size counts records; count counts known measurements.']],
 W20:[
 ...
 V29:['Draw exact total tips by size from df, with one bar per size.','totals = df.groupby("size")["tip"].sum()\nax.bar(totals.index, totals.values)','size','Total tips']
 };
 ...
 revise('I22',0,'Preserve df. Display a DataFrame with one row for each original column. Name its two columns dtype (data type as text) and missing (number of missing values).',
 ...
 revise('I22',2,'Using df, compare average temperature and humidity by sky category for station East only. Display a DataFrame with temperature then humidity columns. Preserve df.',
 ...
 for(const [i,r] of lesson('V37').rounds.entries()){
  ...
  r.steps=[`Using df, keep rows with ${d.a} greater than ${threshold} in selected. Use these same records for all three charts.`,
   ...
   `Figure 3: exact bars of ${mean?'mean':'total'} ${d.a} by ${d.c}. Title ${mean?'Means':'Totals'}; axis labels ${d.c} and ${mean?'Mean '+d.a:'Total'}.`,
