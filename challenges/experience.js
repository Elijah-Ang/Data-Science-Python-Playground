(function(root) {
  "use strict";
  function createChallengeExperience(registry) {
    ...
    function tabs(deck, active) {
      return `<nav class="challenge-mode-tabs" aria-label="Learning mode"><a href="#${deck.id}" ${active === "lessons" ? 'aria-current="page"' : ""}>Lessons &amp; Reviews</a><a href="#${deck.id}/challenges" ${active === "challenges" ? 'aria-current="page"' : ""}>${esc(collectionInfo.title)}</a></nav>`;
    }
    ...
    function page(c, deck, { table, pythonPane, curriculum }) {
      ...
      return `<nav class="foundation-breadcrumb" aria-label="Learning breadcrumb"><a href="${esc(collectionInfo.rootHref)}">${esc(collectionInfo.rootTitle)}</a><span>/</span><a href="#${c.deck}">${esc(deck.title)}</a><span>/</span><a href="#${c.deck}/challenges">${esc(collectionInfo.title)}</a><span>/</span><span>${esc(c.id)}</span></nav><header class="foundation-lesson-heading case-heading"><div class="foundation-lesson-copy"><span class="foundation-eyebrow">${esc(c.id)} \xB7 ${esc(registry.families[c.family])} \xB7 ${esc(c.minutes)}</span><h2>${esc(c.title)}</h2><p>${esc(c.tags.join(" \xB7 "))}</p><button class="foundation-editor-jump" type="button">Go to editor \u2193</button></div><span class="case-deliverable-label">${esc(c.deliverableType)}</span></header><div class="foundation-split"><article class="foundation-content case-brief" aria-label="Challenge brief"><nav class="case-brief-nav" aria-label="Brief sections"><button type="button" data-brief-target="case-inputs">Inputs</button><button type="button" data-brief-target="case-deliverables">Deliverables</button><button type="button" data-brief-target="case-help">Help</button></nav><section class="case-question"><h3>The question</h3><p>${esc(c.question)}</p><div class="case-input-flow">${illustration(c)}<span>${c.inputs.map((i) => esc(i.file || i.name)).join(" + ")} <span aria-hidden="true">\u2192</span> ${esc(c.deliverableType)}</span></div></section><section id="case-inputs" tabindex="-1"><h3>Your inputs</h3>${c.inputs.map((i) => inputPreview(i, table)).join("")}<p class="case-input-note">${c.inputs.some((i) => i.file) ? "The file is available in the Python working folder. Load it yourself." : "The editable setup in Your Python creates these inputs when you run it."}</p></section><section class="case-requirements" id="case-deliverables" tabindex="-1"><h3>Deliverables</h3><p class="case-output-note">Use these variable names and filenames so Check answer can inspect each result.</p><ol>${c.deliverables.map((d) => `<li><div class="case-output-heading"><strong>${esc(d.label)}</strong><span class="case-output-contract"><code>${esc(d.fileName || d.name)}</code><span>${esc(d.format)}</span></span></div><p>${esc(d.requirement)}</p>${d.kind === "figure" ? `<p>${esc(chartLabels(c.chart))}</p>` : ""}</li>`).join("")}</ol><p class="case-policy"><strong>Data policy</strong> ${c.policies.map(esc).join(" ")}</p></section>${c.planning.length ? `<aside class="case-planning"><h3>Before you code</h3>${c.planning.map((p) => `<p>${esc(p)}</p>`).join("")}<small>Optional thinking prompt \xB7 no answer to submit</small></aside>` : ""}<details class="case-help" id="case-help"><summary>Help <span>Hints &amp; explained solution</span></summary><p>Choose the amount of help you need. Opening one hint does not reveal the others.</p><details><summary>Hint 1 \u2014 Think</summary><p>${esc(c.hints.think)}</p></details><details><summary>Hint 2 \u2014 Tools</summary><p>${esc(c.hints.tools)}</p><p class="case-prerequisites">Revisit: ${links}</p></details><details><summary>Hint 3 \u2014 Approach</summary><p>${esc(c.hints.approach)}</p></details><details><summary>Explained solution</summary><p>This complete script replaces the full editor contents, including setup.</p><pre><code>${esc(c.reference)}</code></pre><h4>Why these steps exist</h4><ul>${c.explanationSteps.map((step) => `<li>${esc(step)}</li>`).join("")}</ul><h4>Another valid approach</h4><p>${esc(c.alternative)}</p></details></details></article>${pythonPane(round, false, true)}</div><nav class="foundation-navigation" aria-label="Challenge navigation">${index ? `<a href="${url(entries[index - 1])}">\u2190 Previous challenge</a>` : "<span></span>"}<a href="#${c.deck}/challenges">All challenges</a>${index < entries.length - 1 ? `<a href="${url(entries[index + 1])}">Next challenge \u2192</a>` : "<span></span>"}</nav>`;
    }
    ...
    function bind() {
      document.querySelectorAll("[data-brief-target]").forEach((button) => button.addEventListener("click", () => {
        ...
        if (matchMedia("(min-width: 801px)").matches) {
          brief.scrollTop += target.getBoundingClientRect().top - brief.getBoundingClientRect().top - 12;
  ...
  root.ChallengeExperience = createChallengeExperience(root.DataWorkflowChallenges);
})(window);
