...
(() => {
  ...
  const MODELS = {
    ...
    qda:{name:"Quadratic Discriminant Analysis", family:"Classification", task:"classification", metric:"macro F1 · accuracy", requires:"continuous"},
    ...
    mlp_cls:{name:"Neural Network · classification", family:"Neural Networks", task:"classification", metric:"macro F1 · accuracy"},
    mlp_reg:{name:"Neural Network · regression", family:"Neural Networks", task:"regression", metric:"RMSE · R²"},
    kmeans:{name:"K-Means Clustering", family:"Unsupervised", task:"unsupervised", metric:"silhouette"},
    hierarchical:{name:"Hierarchical Clustering", family:"Unsupervised", task:"unsupervised", metric:"silhouette"},
    pca:{name:"Principal Component Analysis (PCA)", family:"Dimensionality reduction", task:"unsupervised", metric:"variance explained"}
  };
...
`;
  ...
  function compatible(modelId, config, value) {
    ...
    if (model.task !== "unsupervised" && model.task !== config.task) return false;
    if (["mlp_cls","mlp_reg"].includes(modelId) && config.rows < 150) return false;
    if (modelId === "simple_linear") return featureCount(value) === 1 && value.continuous.length === 1;
    if (modelId === "multiple_linear") return featureCount(value) > 1;
    if (modelId === "qda" && featureCount(value) > 10) return false;
    if (["polynomial","lda","qda"].includes(modelId)) return value.continuous.length >= 1 && value.binary.length === 0 && value.categorical.length === 0;
    if (["kmeans","hierarchical","pca"].includes(modelId)) return value.continuous.length >= 2 && value.binary.length === 0 && value.categorical.length === 0;
    return true;
  }
  ...
  function staticSetup() {
    ...
    [model?.task === "unsupervised" ? "unsupervised" : config.task, config.target, value.name, model?.task === "unsupervised" ? "no holdout target" : (config.split === "time" ? "chronological 80 / 20" : "sealed 80 / 20")].forEach(label => {
    ...
    $("#routeDescription").textContent = unsupervised
      ...
      : "Honest model workflow · Run in order; the final block is the only one allowed to inspect the holdout.";
  ...
  const task = (id, title, caption, code) => ({id, title, caption, code:formatRouteCode(code)});
...
pd.DataFrame({
  ...
  function exploreCode(config, value, unsupervised = false) {
    if (featureNames(value).length === 1) {
      ...
      return `# 2 · Explore the one selected predictor
...
summary`;
    ...
    return `# 2 · Explore predictor inputs only; the holdout target remains unseen
summary = model_df[feature_names].describe(include="all").T
print("Missing values in selected inputs:", int(model_df[feature_names].isna().sum().sum()))
...
summary`;
  ...
  function splitCode(config) {
    if (config.split === "time") return `# 3 · Seal the latest 20% as the final test set
...
pd.DataFrame({"partition":["training + CV", "sealed final test"], "rows":[len(X_train), len(X_test)], "order":["earlier", "later"]})`;
    return `# 3 · Seal a 20% holdout before any cross-validation or tuning
from sklearn.model_selection import train_test_split
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.20, random_state=42${config.task === "classification" ? ", stratify=y" : ""}
)
pd.DataFrame({"partition":["training + CV", "sealed final test"], "rows":[len(X_train), len(X_test)]})`;
  }

  function cvCode(config, folds) {
    ...
    return `# 6 · Define ${folds}-fold cross-validation on training rows only
from sklearn.model_selection import ${splitterName}
...
pd.DataFrame(fold_plan)`;
  ...
  function preprocessingCode(value, modelId) {
    ...
    const oneHotDrop = keepOriginalUnits ? "'first'" : "None";
    const continuousStep = noScale || keepOriginalUnits ? "('impute', SimpleImputer(strategy='median'))" : "('impute', SimpleImputer(strategy='median')), ('scale', StandardScaler())";
    return `# 4 · Build type-aware preprocessing inside the pipeline
...
preprocessor = ColumnTransformer([
    ("continuous", Pipeline([${continuousStep}]), continuous_features),
    ("binary", Pipeline([
    ...
    ("categorical", Pipeline([
        ("impute", SimpleImputer(strategy="most_frequent")),
        ${ordinalCategories ? "('ordinal', OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1))" : `('one_hot', OneHotEncoder(handle_unknown='ignore', sparse_output=False, drop=${oneHotDrop}))`}
    ]), categorical_features)
], verbose_feature_names_out=False)
${!ordinalCategories && value.categorical.length ? "# Categories absent from a training fold use OneHotEncoder's safe all-zero fallback." : ""}

pd.DataFrame({
    "feature_type": ["continuous", "binary", "categorical"],
    "columns": [len(continuous_features), len(binary_features), len(categorical_features)],
    "treatment": [
        ${py(noScale || keepOriginalUnits ? "median imputation; original units retained" : "median imputation + standardisation")},
        "mode imputation + 0/1 encoding",
        ${py(ordinalCategories ? "mode imputation + one code per original feature" : "mode imputation + one-hot encoding")}
    ]
})`;
  }
  ...
  function modelSpec(modelId, value) {
    ...
    const specs = {
      ...
      polynomial:{concept:"Expand continuous inputs into curved terms, then regularise", imports:"from sklearn.preprocessing import PolynomialFeatures\nfrom sklearn.linear_model import Ridge", estimator:"Pipeline([('poly', PolynomialFeatures(include_bias=False)), ('regression', Ridge())])", grid:"{'model__poly__degree':[2, 3], 'model__regression__alpha':[0.1, 1.0, 10.0]}"},
      ...
      one_r:{concept:"Use the single feature whose simple rules make the fewest errors", imports:`from sklearn.base import BaseEstimator, ClassifierMixin
class OneRClassifier(ClassifierMixin, BaseEstimator):
    ...
    def predict(self, X):
      ...
      mlp_cls:{concept:"Learn nonlinear layers of weighted features with backpropagation", imports:"from sklearn.neural_network import MLPClassifier", estimator:"MLPClassifier(max_iter=500, early_stopping=True, random_state=42)", grid:"{'model__hidden_layer_sizes':[(24,), (32, 16)], 'model__alpha':[0.0001, 0.01]}"},
      mlp_reg:{concept:"Learn nonlinear layers while scaling the target inside the model", imports:"from sklearn.neural_network import MLPRegressor\nfrom sklearn.compose import TransformedTargetRegressor", estimator:"TransformedTargetRegressor(regressor=MLPRegressor(max_iter=800, early_stopping=True, tol=1e-3, random_state=42), transformer=StandardScaler())", grid:"{'model__regressor__hidden_layer_sizes':[(24,), (32, 16)], 'model__regressor__alpha':[0.0001, 0.01]}"}
    };
...
pd.DataFrame({"component":["preprocessing","model"], "object":[type(preprocessor).__name__, type(estimator).__name__]})`;
  }

  function baselineCode(config) {
    if (config.task === "classification") return `# 7 · Measure the untuned model across every training fold
from sklearn.model_selection import cross_validate
scoring = {"macro_f1":"f1_macro", "accuracy":"accuracy"}
baseline = cross_validate(pipeline, X_train, y_train, cv=cv, scoring=scoring, return_train_score=True, error_score="raise")
...
cv_scores.round(3)`;
    return `# 7 · Measure the untuned model across every training fold
from sklearn.model_selection import cross_validate
...
cv_scores.round(3)`;
  ...
  function tuningCode(config, modelId, value) {
    ...
    const refit = config.task === "classification" ? "macro_f1" : "RMSE";
    return `# 8 · ${hasHyperparameters ? "Tune hyperparameters" : "Validate the model defaults"} inside the same training folds
from sklearn.model_selection import GridSearchCV
parameter_grid = ${spec.grid}
tuned = GridSearchCV(
    pipeline, parameter_grid, cv=cv, scoring=${scoring}, refit=${py(refit)},
    return_train_score=True, n_jobs=1, error_score="raise"
)
tuned.fit(X_train, y_train)
tuning_results = pd.DataFrame(tuned.cv_results_).sort_values("rank_test_${refit}")
${config.task === "regression" ? `tuning_results["mean_train_RMSE"] *= -1
tuning_results["mean_test_RMSE"] *= -1` : ""}
print("Best settings:", tuned.best_params_)
tuning_results[["params", "mean_train_${refit}", "mean_test_${refit}", "rank_test_${refit}"]].head(10).round(3)`;
  }

  function interpretationCode(modelId) {
    if (["simple_linear","multiple_linear"].includes(modelId)) return `
encoded_names = validation_model.named_steps["prepare"].get_feature_names_out()
fitted = validation_model.named_steps["model"]
...
prepared_names = validation_model.named_steps["prepare"].get_feature_names_out()
fitted = validation_model.named_steps["model"]
...
pd.DataFrame(tuned.cv_results_)[["param_model__n_neighbors","param_model__weights","mean_test_macro_f1"]].sort_values("mean_test_macro_f1", ascending=False)`;
    if (modelId === "qda") return `
fitted = validation_model.named_steps["model"]
pd.DataFrame(fitted.means_, index=fitted.classes_, columns=validation_model.named_steps["prepare"].get_feature_names_out()).reset_index(names="class")`;
...
pd.DataFrame(lda_coef, index=lda_rows, columns=validation_model.named_steps["prepare"].get_feature_names_out()).reset_index(names="class_or_boundary")`;
...
wrapped_model = validation_model.named_steps["model"]
...
pd.DataFrame({"layers":[fitted.hidden_layer_sizes], "iterations":[fitted.n_iter_], "final_loss":[fitted.loss_]})`;
  ...
  function diagnosticsCode(config, modelId) {
    ...
    if (config.task === "classification") return `# 9 · Inspect post-tuning cross-validated predictions
from sklearn.base import clone
from sklearn.model_selection import cross_val_predict
from sklearn.metrics import confusion_matrix, classification_report
validation_prediction = cross_val_predict(tuned.best_estimator_, X_train, y_train, cv=cv, method="predict", n_jobs=1)
validation_model = clone(tuned.best_estimator_).fit(X_train, y_train)

fig, ax = plt.subplots(figsize=(5.4, 4.2))
sns.heatmap(confusion_matrix(y_train, validation_prediction), annot=True, fmt="d", cmap="Purples", ax=ax,
            xticklabels=np.unique(y_train), yticklabels=np.unique(y_train))
ax.set(title="Post-tuning CV confusion matrix", xlabel="Predicted", ylabel="Actual")
fig.tight_layout()
report = pd.DataFrame(classification_report(y_train, validation_prediction, output_dict=True, zero_division=0)).T
print(report.round(3))
${interpretation}
`;
    const validation = config.split === "time"
      ? `last_fit, last_validation = list(cv.split(X_train, y_train))[-1]
validation_model = clone(tuned.best_estimator_).fit(X_train.iloc[last_fit], y_train.iloc[last_fit])
validation_actual = y_train.iloc[last_validation]
validation_prediction = validation_model.predict(X_train.iloc[last_validation])`
      : `validation_prediction = cross_val_predict(tuned.best_estimator_, X_train, y_train, cv=cv, method="predict", n_jobs=1)
validation_actual = y_train
validation_model = clone(tuned.best_estimator_).fit(X_train, y_train)`;
    return `# 9 · Diagnose post-tuning validation residuals before the test set
from sklearn.base import clone
from sklearn.model_selection import cross_val_predict
from sklearn.metrics import root_mean_squared_error, r2_score
${validation}
residuals = validation_actual.to_numpy() - validation_prediction
fig, axes = plt.subplots(1, 2, figsize=(10, 3.8))
sns.scatterplot(x=validation_prediction, y=residuals, ax=axes[0], color="#7651a6")
axes[0].axhline(0, color="#c75b20", linestyle="--")
axes[0].set(title="Validation residuals", xlabel="prediction", ylabel="actual − prediction")
sns.histplot(residuals, kde=True, ax=axes[1], color="#137c9c")
axes[1].set_title("Residual distribution")
fig.tight_layout()
print("Validation RMSE:", round(root_mean_squared_error(validation_actual, validation_prediction), 3))
print("Validation R²:", round(r2_score(validation_actual, validation_prediction), 3))
${interpretation}`;
  }

  function finalCode(config) {
    if (config.task === "classification") return `# 10 · Refit on all training rows, then open the test set exactly once
from sklearn.metrics import accuracy_score, f1_score, classification_report, confusion_matrix
final_model = tuned.best_estimator_.fit(X_train, y_train)
...
test_result = pd.DataFrame({
    ...
    "value":[f1_score(y_test, test_prediction, average="macro"), accuracy_score(y_test, test_prediction), len(y_test)]
})
test_result.round(3)`;
    return `# 10 · Refit on all training rows, then open the test set exactly once
...
test_result = pd.DataFrame({
    ...
    "value":[root_mean_squared_error(y_test, test_prediction), mean_absolute_error(y_test, test_prediction), r2_score(y_test, test_prediction), len(y_test)]
})
test_result.round(3)`;
  }
  ...
  function supervisedRoute(config, value, modelId, folds) {
    ...
    return [
      task("frame","Frame problem","Choose X and y",frameCode(config, value)),
      task("explore","Explore inputs","pandas + seaborn",exploreCode(config, value)),
      task("split","Seal holdout",config.split === "time" ? "latest 20%" : "stratified / random 20%",splitCode(config)),
      task("prepare","Preprocess","inside every fold",preprocessingCode(value, modelId)),
      task("model","Build model",modelSpec(modelId, value).concept,modelCode(modelId, value)),
      task("folds",`${folds}-fold plan`,config.split === "time" ? "walk forward" : "training only",cvCode(config, folds)),
      task("baseline","Untuned CV",`${folds} validation scores`,baselineCode(config)),
      task("tune",hasHyperparameters ? "Tune" : "Validate defaults",`GridSearchCV · ${folds} folds`,tuningCode(config, modelId, value)),
      task("diagnose","Diagnose","post-tuning CV · training only",diagnosticsCode(config, modelId)),
      task("final","Final test","open sealed 20% once",finalCode(config))
    ];
  }
  ...
  function clusterPreprocessing(value) {
    return `# 3 · Impute and standardise the continuous inputs
from sklearn.impute import SimpleImputer
from sklearn.pipeline import Pipeline
...
encoded_names = np.array(feature_names)
...
pd.DataFrame({"rows":[Z.shape[0]], "scaled_dimensions":[Z.shape[1]]})`;
  ...
  function kmeansRoute(config, value) {
    return [
      task("frame","Frame discovery","target stays hidden",unsupervisedFrameCode(config, value)),
      task("explore","Explore structure","pandas + seaborn",exploreCode(config, value, true)),
      task("prepare","Scale + encode","distance-ready matrix",clusterPreprocessing(value)),
      task("compare","Compare k","exploratory inertia + silhouette",`# 4 · Compare k on the same data; this is exploratory, not a test score
...
pd.DataFrame({"selected_k":[best_k], "silhouette":[silhouette_score(Z, clusters)], "inertia":[kmeans.inertia_]}).round(3)`),
      task("diagnose","Check clusters","size + separation",`# 6 · Check whether the solution is balanced and separated
...
cluster_quality.round(3)`),
      task("profile","Profile clusters","translate numbers to meaning",`# 7 · Translate cluster IDs back into the original features
...
cluster_profile.reset_index()`),
      task("visualise","Map clusters","PCA view only",`# 8 · Project to two dimensions for a visual map (the model used all dimensions)
from sklearn.decomposition import PCA
projection = PCA(n_components=2, random_state=42).fit_transform(Z)
...
analysis_rows = model_df.iloc[sample_index].copy()`),
      task("dendrogram","Build dendrogram","Ward linkage sample",`# 4 · Inspect the hierarchy before choosing a cut
...
fig.tight_layout()
pd.DataFrame(linkage_matrix[-10:], columns=["left_group","right_group","merge_distance","members"]).round(2)`),
      task("compare","Choose cut","exploratory silhouette",`# 5 · Compare cuts on the same sample; this is exploratory, not a test score
from sklearn.cluster import AgglomerativeClustering
from sklearn.metrics import silhouette_score
candidate_rows = []
for k in range(2, min(9, len(analysis_Z))):
...
fig.tight_layout()
candidate_scores.round(3)`),
      task("fit","Fit hierarchy","Ward agglomeration",`# 6 · Fit the selected hierarchy
...
pd.Series(clusters).value_counts().sort_index().rename_axis("cluster").reset_index(name="rows")`),
      task("profile","Profile groups","feature means",`# 7 · Describe the discovered groups in original units
profile_df = analysis_rows[feature_names].copy()
profile_df["cluster"] = clusters
cluster_profile = profile_df.groupby("cluster")[feature_names].mean().round(2)
cluster_profile.reset_index()`),
      task("visualise","Map hierarchy","PCA teaching view",`# 8 · Visualise the sampled hierarchy in two dimensions
from sklearn.decomposition import PCA
projection = PCA(n_components=2, random_state=42).fit_transform(analysis_Z)
plot_df = pd.DataFrame({"PC1":projection[:,0], "PC2":projection[:,1], "cluster":clusters.astype(str)})
fig, ax = plt.subplots(figsize=(6.5, 4.5))
sns.scatterplot(data=plot_df, x="PC1", y="PC2", hue="cluster", palette="tab10", alpha=.75, ax=ax)
...
plot_df.head(12)`)
  ...
  function pcaRoute(config, value) {
    return [
      task("frame","Frame reduction","target stays hidden",unsupervisedFrameCode(config, value)),
      task("explore","Inspect correlation","redundancy + scale",`# 2 · Inspect redundancy and unequal scales before PCA
numeric_view = model_df[continuous_features].copy() if continuous_features else pd.DataFrame()
if len(continuous_features) >= 2:
    fig, ax = plt.subplots(figsize=(7, 5))
    sns.heatmap(numeric_view.corr(), cmap="vlag", center=0, ax=ax)
    ax.set_title("Correlation among continuous inputs")
    fig.tight_layout()
model_df[feature_names].describe(include="all").T`),
      task("prepare","Scale + encode","comparable dimensions",clusterPreprocessing(value)),
      task("fit","Fit full PCA","all possible components",`# 4 · Fit PCA without guessing the component count
from sklearn.decomposition import PCA
full_pca = PCA().fit(Z)
...
variance_table.round(4)`),
      task("scree","Read scree plot","elbow + cumulative rule",`# 5 · Read individual and cumulative explained variance
...
fig.tight_layout()
variance_table.round(4)`),
      task("select","Select components","retain at least 90%",`# 6 · Select the smallest representation retaining at least 90% variance
...
pd.DataFrame({"original_dimensions":[Z.shape[1]], "selected_components":[n_components_90], "variance_retained":[pca.explained_variance_ratio_.sum()]}).round(4)`),
      task("loadings","Interpret loadings","what each axis combines",`# 7 · Interpret the largest positive and negative loadings
loadings = pd.DataFrame(full_pca.components_.T, index=encoded_names, columns=[f"PC{i}" for i in range(1, len(full_pca.components_) + 1)])
loading_view = loadings[["PC1", "PC2"]].copy() if loadings.shape[1] >= 2 else loadings[["PC1"]].copy()
loading_view["largest_absolute_loading"] = loading_view.abs().max(axis=1)
loading_view.sort_values("largest_absolute_loading", ascending=False).head(20).round(3)`),
      task("project","Project rows","2D teaching view",`# 8 · Project rows, then add labels only for interpretation
...
plot_df.head(12)`)
  ...
  function renderRoute() {
    ...
    routeTasks.forEach((item, index) => {
      ...
      button.title = `${item.title} — ${item.caption}`;
...
explore_df.head(10)`;
  ...
  function renderNotebook() {
    ...
    cells.forEach(cell => {
      ...
      const run = document.createElement("button"); run.type = "button"; run.className = "cell-action run"; run.textContent = cell.status === "running" ? "running…" : "▶ run"; run.disabled = cell.status === "running"; run.addEventListener("click", () => runCell(cell));
  ...
  function updateSeal() {
    const unsupervised = selectedModel()?.task === "unsupervised", used = cells.some(cell => cell.stage === "final" && cell.status === "done");
    ...
    $(".privacy-note").textContent = unsupervised
      ...
      : "The 20% holdout stays sealed while cross-validation and tuning use only the training portion.";
  }

  function clearNotebook(message = "Notebook cleared; the test set is sealed again.") {
    cells = []; cellSequence = 0; latestChart = null;
    $("#outputStatus").textContent = "No cell run yet";
    renderNotebookView(); renderRoute(); updateSeal();
    if (message) showToast(message);
  }

  function rebuildSetup({scenarioChanged = false} = {}) {
  ...
  async function loadDataset(id) {
    ...
    $("#runtimeStatus").textContent = `Reading ${selectedConfig().name}…`;
  ...
  function workflowNarrative(item) {
    const modelId = selectedModelId();
    const notes = {
      frame:"Name the modelling rows, selected predictors and outcome before fitting anything.",
      explore:"Inspect only the available predictor inputs so the sealed outcome cannot shape early decisions.",
      split:"Set aside the final 20% now. Cross-validation, preprocessing and tuning must use the training portion only.",
      prepare:"Put every type-specific transformation inside the reusable preprocessing object so each fold learns from its own training rows.",
      model:"Create the selected estimator and attach it after preprocessing in one pipeline.",
      folds:"Define the exact validation splits used by both the untuned check and hyperparameter search.",
      baseline:"Measure the unchanged pipeline across every validation fold before tuning it.",
      tune:"Search only inside the training folds, select by the stated primary metric, and retain the full comparison table.",
      diagnose:"Study post-tuning training-fold predictions and model-specific evidence without opening the test set.",
      final:"Refit the chosen pipeline on all training rows, open the sealed test set once, and report the final metrics.",
      compare:"Compare candidate structures on the same prepared matrix; treat the result as exploratory rather than a test score.",
      dendrogram:"Build the Ward-linkage hierarchy on one reproducible sample before choosing where to cut it.",
      fit:modelId === "pca" ? "Fit PCA across all prepared dimensions before deciding how many components to keep." : "Fit the selected exploratory structure using the choice made in the previous step.",
      scree:"Read individual and cumulative explained variance to see how quickly information concentrates.",
      select:"Keep the smallest number of components that meets the stated variance threshold.",
      loadings:"Connect the abstract principal components back to the original scaled inputs.",
      profile:"Describe the discovered groups in the original feature units so cluster labels gain practical meaning.",
      visualise:"Use a two-dimensional PCA view only for interpretation; the clustering model still used every prepared dimension.",
      project:"Project rows after PCA is fitted, then add the reference outcome only as an interpretation layer."
    };
    return notes[item.id] || "Complete this step using the exact code shown, then inspect its output before continuing.";
  }
  ...
  function renderWorkflow() {
    ...
    intro.innerHTML = "Scroll from framing to the final result. <strong>Every title, step and code block below comes directly from the current Suggested Route.</strong> Run the cells in order, then keep this window beside the notebook and type each block yourself.";
    ...
    routeTasks.forEach((item, index) => {
      ...
      const note = document.createElement("p"); note.className = "workflow-step-note"; note.textContent = workflowNarrative(item);
      const typeNote = document.createElement("p"); typeNote.className = "workflow-type-note"; typeNote.textContent = "Type this exact cell";
    ...
    foot.textContent = "Drag the header to move this reference. Pull any edge or corner to resize it. Changing the dataset, feature scenario, model or fold count rebuilds this workflow from the same source as the route above.";
  ...
  $("#scenarioSelect").addEventListener("change", () => rebuildSetup({scenarioChanged:true}));
  $("#modelSelect").addEventListener("change", () => rebuildSetup());
  $("#foldSelect").addEventListener("change", () => rebuildSetup());
