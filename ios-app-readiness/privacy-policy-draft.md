...
- CSV files, dataset values, Python code, notebook inputs, model settings, charts, and generated results supplied by the user.
...
The current product is designed to process uploaded data locally in the browser or app web view. In the current design, uploaded CSVs and analysis inputs are not sent to a project-operated server by default. This statement must be re-checked against the final native build and every integrated SDK before publication.
...
The project does not intentionally sell or rent users’ uploaded datasets. A user may choose to share an exported file or chart through the browser, Files app, or system share sheet; that sharing is controlled by the user and the selected destination.
...
Reasonable measures will be used to protect the project and its release infrastructure. No internet-connected software can guarantee absolute security. Users should keep their devices and operating systems updated and should not import data they do not have permission to use.
