...
Every teaching card has an explicit visual definition: **89/89**. The visual-accuracy follow-up reviewed all **106 cards and 326 exercise mappings**, replacing generic or misleading sketches. Eight rendering families—table, table-transform, bars, distribution, relationship, matrix, panels and figure—share deterministic SVG/table/axis/mark primitives. There are 106 reviewed concept scenes, including alternate-round operations, plus retrieval compositions derived from actual source skills. Three checkpoints show their specific workflows. Exercise visuals now follow the active round, including retrieval tasks. Unknown configurations fail validation. See [the complete per-card visual audit](foundations-visual-audit.md).
