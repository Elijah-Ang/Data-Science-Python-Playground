...
For substantial Machine Learning Playground changes, use the lightweight release path: work on a feature branch, open a pull request, wait for the structural and representative ML audits to pass, then merge to `main`. The Pages workflow repeats that gate before publishing. The full 254-route runtime audit is available from the ML audit workflow's `workflow_dispatch` input and runs on the weekly schedule. Branch protection is not currently configured, so the pull-request/merge check remains a project-maintainer responsibility.
