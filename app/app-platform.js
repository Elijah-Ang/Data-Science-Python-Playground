...
async function configureNativeChrome() {
  ...
  const backgroundColor = getComputedStyle(document.documentElement)
    .getPropertyValue("--chrome-bg")
    .trim() || (theme === "dark" ? "#0c1125" : "#efe8d7");
  if (!Capacitor.isPluginAvailable("StatusBar")) {
    console.error("[AppPlatform] StatusBar plugin is unavailable; native chrome cannot follow the page theme.");
    return;
  }
  const runStatusBarStep = async (name, operation) => {
    try { await operation(); }
    catch (error) { console.error(`[AppPlatform] StatusBar ${name} failed:`, error); }
  };
  await runStatusBarStep("overlay configuration", () => StatusBar.setOverlaysWebView({ overlay: false }));
  await runStatusBarStep("style configuration", () => StatusBar.setStyle({ style: theme === "dark" ? Style.Dark : Style.Light }));
  await runStatusBarStep("background configuration", () => StatusBar.setBackgroundColor({ color: backgroundColor }));
  await runStatusBarStep("verification", async () => {
    const info = await StatusBar.getInfo();
    if (info?.color && info.color.toLowerCase() !== backgroundColor.toLowerCase()) {
      console.warn(`[AppPlatform] StatusBar color mismatch: expected ${backgroundColor}, received ${info.color}.`);
    }
  });
